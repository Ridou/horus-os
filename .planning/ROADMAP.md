...
**Execution Order (v0.7):** 60 → (61 ∥ 63) → 62 → (64 ∥ 65 ∥ 66) → 67 → 68
...
**Milestone Goal:** Turn the plain local dashboard into a polished command center, ship a named starter team and seed content, and add an entirely optional integration suite (Discord control bot, Supabase sync, Vercel deploy observer, Tailscale remote access, GitHub, AI providers, and existing adapters) connectable through guided in-dashboard walkthroughs with green-light verification and in-app key management. Every integration is optional; the system starts with only a local Python process and a SQLite database.

**Execution Order:** 60 → (61 ∥ 63) → 62 → (63 if not yet done ∥ 64 ∥ 65 ∥ 66) → 67 → 68. Phase 60 is a universal prerequisite: design tokens, Modal/Stepper primitives, and the AppShell sidebar are required by every page and by the Setup-and-Verify UX. Phase 61 (read-only integrations API) must land before Phase 62 (secret-aware key-write) so the write surface can be security-reviewed in isolation. Phase 63 (pages and starter team) depends only on Phase 60 and can run concurrently with Phases 61 and 62. Phases 64, 65, 66 run largely in parallel after Phase 62 provides the key-write endpoint; they share only the task API and SQLite. Phase 67 (Vercel + GitHub tool) depends on Phase 65 (Supabase as data source) and requires all prior security work before any docs instruct public exposure. Phase 68 (release) is last, behind the three-OS hard gate.

**Eight load-bearing constraints carried across phases:**

1. REMOTE-01 (pin starlette >= 1.0.1 and loopback guard) lands in Phase 62 BEFORE the key-write endpoint is callable - unauthenticated /api on any public-facing bind is the BLOCKING security item per PITFALLS.md Pitfall 1.
2. SUPA-02 (service key never in NEXT_PUBLIC_*) and SUPA-03 (RLS on every table) land together in Phase 65 BEFORE SUPA-01 (sync loop) is wired - Pitfall 2 (service-role key in static bundle) and Pitfall 3 (RLS off) are both BLOCKING.
3. DISC-05 (idempotent non-destructive channel bootstrap) is the FIRST design decision in Phase 64 - bootstrap code may not be written until this constraint is formally established and test-covered (Pitfall 4).
4. Every integration is optional. No phase may make horus-os require a cloud account to start locally. SUPA-05 and the REMOTE-04/05 escape hatches enforce this.
5. The NEXT_PUBLIC_API_BASE abstraction (Pitfall 11 / VERCEL-01) is wired from the first commit of Phase 67 - zero bare fetch("/api/") calls may exist after Phase 67 ships.
6. Secret-write response never echoes the written value; demo-mode returns 403; chmod 600 on the .env write (Pitfalls 12, 13, KEYS-02/03).
7. CI em-dash grep and private-name grep enforced from Phase 60 onward per PITFALLS.md Pitfall 18 and CLAUDE.md hard rule 3.
8. Three-OS hard gate (macOS, Ubuntu, Windows; Python 3.11 and 3.12) is a non-negotiable pre-release condition for Phase 68 (REL-16).

- [x] **Phase 60: Design system and layout shell** - Tailwind v4 tokens, Radix headless primitives (Modal, Stepper), AppShell sidebar navigation, dark theme, shared UI kit. (completed 2026-05-31)
- [x] **Phase 61: Integrations surface and read-only API** - GET /api/integrations, per-integration verify probe, Integrations page with status-dot cards, guided multi-step walkthrough UX (read-only), get-started page, GitHub integration card. (completed 2026-05-31)
- [x] **Phase 62: Key-write endpoint and verification engine** - POST /api/integrations/{name}/keys with loopback guard, starlette pin, chmod 600; integration verification state table; masked settings page; key-hash change detection; TEST-27 regression suite. (completed 2026-05-31)
- [x] **Phase 63: Tier-1 dashboard pages, starter team, and seed content** - /team, /team/[agent], /memory, /tasks, /activity, /traces, /costs, /about; five starter agents with SOUL.md; example vault; demo trace; first-run onboarding tour. (completed 2026-05-31)
- [x] **Phase 64: Discord control bot extension** - Idempotent channel bootstrap, #horus thread flow, guild-scoped slash commands, task-status cards, reaction feedback, admin role gate, setup walkthrough; TEST-28 idempotency test. (completed 2026-05-31)
- [x] **Phase 65: Supabase sync loop and schema migrations** - supabase_sync.py asyncio task, RLS migrations, sync_health table, Supabase-backed dashboard read path, [supabase] extra; TEST-29 service-key exposure test. (completed 2026-05-31)
- [x] **Phase 66: Cron scheduler and always-on service** - scheduler.py with catch-up policy, schedules table, horus-os service install cross-platform, Tailscale remote-access docs, REMOTE-02 docs; TEST-30 cross-OS service install test. (completed 2026-06-02)
- [x] **Phase 67: Vercel deploy path, GitHub tool, NEXT_PUBLIC_API_BASE** - NEXT_PUBLIC_API_BASE throughout api.ts, Vercel deploy walkthrough, GET /api/integrations/vercel/status, [vercel] extra, GitHub tool behind [github] extra; TEST-31 install-smoke with new extras. (completed 2026-06-02)
- [x] **Phase 68: Three-OS gate and v0.7.0 release** - Three-OS hard gate (macOS, Ubuntu, Windows x Python 3.11/3.12), new optional extras documented, REL-16 and REL-17 delivered. (completed 2026-06-02)

## Phase Details: v0.7 Command Center

### Phase 60: Design system and layout shell

**Goal**: A unified design language and layout shell are available so every subsequent page and wizard can be built against them without reinventing primitives.
**Depends on**: Phase 59 (v0.6 contribution gate)
**Requirements**: DESIGN-01, DESIGN-02, DESIGN-03, DESIGN-04, DESIGN-05
**Success Criteria** (what must be TRUE):

  1. Every page in the dashboard renders against a single set of Tailwind v4 CSS variable tokens (background, text, accent, border, semantic) defined in one file with no duplication.
  2. A developer can open a Modal from any page by importing a single Modal primitive - it portal-mounts, traps focus, and closes on Escape.
  3. A developer can embed a multi-step Stepper in any modal to walk a user through numbered steps with back/continue/done controls.
  4. The dashboard loads with a persistent sidebar navigation shell listing all routes; the active route is highlighted; dark theme is the default.
  5. Empty state, loading skeleton, status dot/badge, metric card, and markdown renderer components are present in the shared UI kit and used at least once each.
...
- [x] 60-01-PLAN.md - radix-ui + vitest/jsdom test stack, finalize globals.css token source, migrate hardcoded hex and off-spec font weights (DESIGN-01)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 60-02-PLAN.md - Modal (Radix Dialog wrapper) and four-state StatusDot primitives with jsdom tests, barrel exports (DESIGN-02, DESIGN-05)
- [x] 60-03-PLAN.md - Sidebar ten-item nav with active highlight, /tasks and /integrations stub pages, em-dash and private-name CI guards (DESIGN-04, DESIGN-01, DESIGN-05)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 60-04-PLAN.md - controlled Stepper primitive plus extracted CopyButton/CommandBlock with jsdom tests, barrel exports (DESIGN-03, DESIGN-05)

### Phase 61: Integrations surface and read-only API

**Goal**: Users can see every available connector in one place with live status indicators and step through a guided walkthrough to understand what each integration provides and where to get credentials - all without any secret being written.
**Depends on**: Phase 60
**Requirements**: SETUP-01, SETUP-02, SETUP-03, SETUP-04, SETUP-05, SETUP-06, VERIFY-02, VERIFY-04, GH-01
**Success Criteria** (what must be TRUE):

  1. GET /api/integrations returns each integration's metadata and live configured/verified status without returning any secret value.
  2. The Integrations page renders a card for each connector (Anthropic, Gemini, Discord, Slack, Email, Calendar, GitHub, Supabase, Vercel, Tailscale) with a color-coded status dot indicating its state.
  3. Clicking any integration card opens a multi-step guided popup explaining what it unlocks, where to get the credential (with a portal deep-link), and the exact env var or command - with no write action possible in this phase.
  4. The get-started page launches the same guided setup walkthrough, replacing the reserved placeholder.
  5. In demo mode, walkthroughs degrade to instructional-only with a visible banner and no write controls rendered.
...
- [x] 61-01-PLAN.md: Read-only backend - integration registry, Pydantic models, GET /api/integrations with no-secret-echo enforcement

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 61-02-PLAN.md: Integrations page and components - card grid, guided walkthrough (Modal + Stepper), readiness summary, demo-mode degradation

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 61-03-PLAN.md: Get-started integration - Connect-your-integrations step linking to the guided setup, placeholder removed

### Phase 62: Key-write endpoint and verification engine

**Goal**: Users can save and rotate credentials from within the dashboard - with all security constraints enforced: loopback-only writes, chmod 600 storage, no secret echoing, demo-mode refusal, and key-hash change detection that invalidates stale green states.
**Depends on**: Phase 61
**Requirements**: KEYS-01, KEYS-02, KEYS-03, KEYS-04, VERIFY-01, VERIFY-03, REMOTE-01, MIG-06, TEST-27
**Success Criteria** (what must be TRUE):

  1. POST /api/integrations/{name}/keys writes the credential to data_dir/.env at chmod 600, never echoes the value in the response, returns HTTP 403 from non-loopback clients, and returns HTTP 403 in demo mode.
  2. Each integration exposes a verification probe reachable via POST /api/integrations/{name}/verify that returns pass or fail without echoing the secret.
  3. The /settings page shows each credential masked and lets the user replace it; the UI shows a restart-required banner when appropriate and immediately reflects the new verification state.
  4. Rotating a credential (key-hash change) automatically invalidates a previously verified green state, requiring the user to re-verify.
  5. TEST-27 regression tests pass: loopback guard, demo-mode refusal, GET /api/integrations no-secret rule, and the new v0.7 schema migration are all covered.
...
- [x] 62-01-PLAN.md - v7 to v8 schema migration: integration_verification_state table + key-hash invalidation methods (MIG-06, VERIFY-03)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 62-02-PLAN.md - key-write + verify endpoints: loopback guard, demo refusal, chmod 600, no-echo, starlette pin, TEST-27 suite (KEYS-02/03, VERIFY-01/03, REMOTE-01, TEST-27)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 62-03-PLAN.md - /settings credential UI: masked rows, replace form, verify, restart banner, demo-mode absence (KEYS-01, KEYS-04, VERIFY-01)

### Phase 63: Tier-1 dashboard pages, starter team, and seed content

**Goal**: The dashboard is a complete, usable product with all tier-1 pages populated and non-empty on first run, including a named starter team and an onboarding tour.
**Depends on**: Phase 60
**Requirements**: PAGES-01, PAGES-02, PAGES-03, PAGES-04, PAGES-05, PAGES-06, PAGES-07, PAGES-08, TEAM-01, TEAM-02, TEAM-03, SEED-01, SEED-02, SEED-03
**Success Criteria** (what must be TRUE):

  1. horus-os init auto-creates five generic starter agents (Coordinator, Engineer, Researcher, Writer, Operator), each with a SOUL.md persona, color, domain, and default model, so /team shows a non-empty roster on first run.
  2. Users can navigate all eight pages (/team, /team/[agent], /memory, /tasks, /activity, /traces, /costs, /about) without encountering empty states that have no content.
  3. /memory shows a browsable example vault of generic sample notes clearly labeled as example data; /traces shows a demo trace with an example-data banner and a way to clear it.
  4. A first-run onboarding tour overlay highlights the core pages in sequence and can be dismissed and replayed from /about.
  5. The Coordinator agent can delegate work to the other four starter agents via the existing delegate_to_agent tool.
...
- [x] 63-01-PLAN.md - tasks table + v8->v9 migration + schema-version tripwire update + storage methods

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 63-02-PLAN.md - tasks/trace-delete routes, demo task seeding, is_example flag, Coordinator delegation test

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 63-03-PLAN.md - frontend tasks data layer (types, api, hooks, query-key, fixture)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 63-04-PLAN.md - ExampleDataBanner, TourProvider/useTour, TourOverlay + layout wiring

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 63-05-PLAN.md - /tasks page + /team/[slug] route + font-weight normalize
- [x] 63-06-PLAN.md - ExampleDataBanner + tour-replay wiring on /traces, /memory, /about + tour-step targets

**UI hint**: yes

### Phase 64: Discord control bot extension

**Goal**: Users can opt in to a Discord control bot that mirrors the local dashboard over a guild, with fully idempotent setup, the #horus thread-dispatch flow, slash commands, outbound task-status cards, and reaction feedback - without ever requiring Discord to use horus-os.
**Depends on**: Phase 62
**Requirements**: DISC-05, DISC-06, DISC-07, DISC-08, DISC-09, DISC-10, TEST-28
**Success Criteria** (what must be TRUE):

  1. Running /horus setup in Discord triggers channel bootstrap that only creates channels that do not exist and never deletes channels outside the bot's managed set; TEST-28 asserts idempotency across multiple bootstrap runs.
  2. Typing a message in the #horus channel opens a thread, routes the message to the orchestrator, and posts the result back in that thread.
  3. Guild-scoped slash commands (/status, /cancel, /horus setup) are registered within seconds of bot startup; no duplicate commands appear; stale global commands are cleared.
  4. Task progress posts as a status card embed in the relevant thread; thumbs-up and thumbs-down reactions write feedback records.
  5. Privileged commands (bootstrap) require the user to hold the configured admin role; the in-dashboard and docs walkthrough covers full-admin bot creation, Message Content intent, and the minimal-permission invite URL.

**Plans**: 3 plans

  - [x] 64-01-PLAN.md - Additive v9->v10 discord_feedback migration + Database helpers; bump [discord] to >=2.4 and exclude from [all]; update all schema-version tripwires 9 -> 10 (Wave 1)
  - [x] 64-02-PLAN.md - Discord control bot: idempotent non-destructive bootstrap (TEST-28), guild-scoped slash sync, #horus thread dispatch, status-card embeds, reaction feedback, admin-role gate (Wave 2)
  - [x] 64-03-PLAN.md - Expand discord integration registry required_vars + control-bot description (walkthrough), regression test, and the v0.7 DISCORD.md setup guide (intents, minimal-permission invite 309237738560, slash commands, admin role) (Wave 2)

### Phase 65: Supabase sync loop and schema migrations

**Goal**: Users who opt in to Supabase get an incremental push-only sync of their local SQLite data to Supabase with RLS-protected schema migrations, so a Vercel-deployed dashboard can display live data - while the local runtime starts and runs fully without any Supabase configuration.
**Depends on**: Phase 62
**Requirements**: SUPA-01, SUPA-02, SUPA-03, SUPA-04, SUPA-05, TEST-29
**Success Criteria** (what must be TRUE):

  1. horus-os serve starts, passes the health check, and shows no errors with zero Supabase env vars set.
  2. When SUPABASE_URL and SUPABASE_SERVICE_KEY are set, the background sync loop incrementally pushes new SQLite rows to Supabase using an updated_at cursor; a sync_health table shows last_synced_at.
  3. The Supabase service key never appears in any NEXT_PUBLIC_* variable, any static bundle artifact, or any browser-exposed response; TEST-29 asserts this at CI time.
  4. Every table created by the Supabase migration has ENABLE ROW LEVEL SECURITY and a service_role_only policy; horus-os doctor --supabase reports all tables RLS-enabled.
  5. When configured, the dashboard reads from Supabase with the anon key so a Vercel-deployed copy shows current data without redeploys.
...
- [x] 65-01-PLAN.md - v10->v11 sync_cursors migration, schema-version tripwires, [supabase] extra + adapter entry point

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 65-02-PLAN.md - SupabaseAdapter: env-guarded lazy-httpx asyncio push-only sync loop (SUPA-01, SUPA-05)
- [x] 65-03-PLAN.md - RLS migration SQL, TEST-29 secret-safety grep, doctor --supabase RPC, docs/SUPABASE.md (SUPA-02, SUPA-03, TEST-29)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 65-04-PLAN.md - minimal frontend anon-read client wiring (SUPA-04)

### Phase 66: Cron scheduler and always-on service

**Goal**: Users can schedule recurring agent routines that fire 24/7 and install horus-os as a platform-native always-on service, with clear cross-platform guidance and a catch-up policy that handles process suspension without flooding agents.
**Depends on**: Phase 62
**Requirements**: REMOTE-02, REMOTE-03, REMOTE-04, REMOTE-05, TEST-30
**Success Criteria** (what must be TRUE):

  1. horus-os service install registers a platform-native service on macOS (launchd), Ubuntu (systemd), and Windows (NSSM wrapper); horus-os doctor --service reports the correct status on each OS; TEST-30 covers all three.
  2. An in-process cron scheduler reads the schedules table and fires agent routines at their configured times using max(croniter_next, now) so a single missed run fires immediately on wake-up without flooding the queue.
  3. Setting HORUS_OS_DISABLE_SCHEDULER disables the cron scheduler without affecting any other functionality.
  4. docs/REMOTE.md documents horus-os serve --host alongside its security implications and covers reaching the dashboard over Tailscale, gated on an auth layer being in place first.
  5. The Windows always-on guide uses OpenSSH for remote management, not Tailscale SSH, and the Linux/macOS guide uses the appropriate systemd/launchd path.
...
- [x] 66-01-PLAN.md: schema v11->v12 schedules table + accessors, croniter base dep, tripwire bump, Wave 0 test stubs

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 66-02-PLAN.md: SchedulerAdapter (coalesce, tz, overlap/double-fire guards, disable gate) + schedule CLI + entry-point

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 66-03-PLAN.md: pure service-definition generators + per-OS manager + service CLI (--print) + doctor --service (TEST-30)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 66-04-PLAN.md: docs/REMOTE.md (serve --host, Tailscale, funnel warning, OpenSSH, service guide) + docs grep test

### Phase 67: Vercel deploy path, GitHub tool, and NEXT_PUBLIC_API_BASE

**Goal**: Users can deploy their own dashboard copy to Vercel pointing at a reachable backend, observe Vercel deploy status from within horus-os, and use an optional GitHub tool for agent repository interaction - all with zero bare /api/ fetch calls remaining in the static bundle.
**Depends on**: Phase 65
**Requirements**: VERCEL-01, VERCEL-02, VERCEL-03, VERCEL-04, GH-02, TEST-31
**Success Criteria** (what must be TRUE):

  1. All fetch() calls in the Next.js frontend use the NEXT_PUBLIC_API_BASE prefix; grep for fetch("/api/ returns zero matches.
  2. A user following docs/DEPLOY-VERCEL.md can configure NEXT_PUBLIC_API_BASE plus the Supabase anon key and deploy a working Vercel-hosted dashboard that reads live data.
  3. GET /api/integrations/vercel/status returns deploy status using the server-side Vercel token without the token reaching any browser response.
  4. Any guidance that exposes the dashboard beyond localhost carries a prominent red-banner warning that /api has no authentication layer; the walkthrough makes adding auth a prerequisite step.
  5. An agent can use the optional GitHub tool (behind [github] extra) to read repository data; TEST-31 install-smoke stays green with the new [supabase], [vercel], and [github] extras present.
...
- [x] 67-01-PLAN.md: Wire NEXT_PUBLIC_API_BASE through api.ts API_BASE constant + vitest guard (VERCEL-01)
- [x] 67-02-PLAN.md: GET /api/integrations/vercel/status + HORUS_OS_VERCEL_TOKEN isolation test (VERCEL-02, VERCEL-03)
- [x] 67-03-PLAN.md: GitHub read tool behind [github] + [vercel]/[github] extras excluded from [all] + install-smoke (GH-02, TEST-31)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 67-04-PLAN.md: docs/DEPLOY-VERCEL.md no-auth-first deploy walkthrough + docs grep test (VERCEL-02, VERCEL-04)

### Phase 68: Three-OS gate and v0.7.0 release

**Goal**: v0.7.0 ships behind the three-OS hard gate with the new optional extras documented and excluded from [all], giving users a complete polished command center with full optional integration suite.
**Depends on**: Phase 67 and all Phase 64, 65, 66 work complete
**Requirements**: REL-16, REL-17
**Success Criteria** (what must be TRUE):

  1. CI green on macOS, Ubuntu, and Windows for both Python 3.11 and 3.12 with all new optional extras installed.
  2. [supabase] and [vercel] extras are documented in README and pyproject.toml and excluded from [all]; pip install horus-os installs neither.
  3. Tag v0.7.0 is published with a GitHub Release containing CHANGELOG, migration notes, and documentation of all new optional extras.
...
- [x] 68-01-PLAN.md: Version bump to 0.7.0, README optional-extras section, [all]-exclusion test (REL-16, REL-17)
- [x] 68-02-PLAN.md: CHANGELOG [0.7.0] entry, v0.5-to-v0.7 migration note, docs grep test (REL-17)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 68-03-PLAN.md: Three-OS CI-matrix test, local release-gate proxy, drafted GitHub Release body (REL-16, REL-17)

| 60. Design system and layout shell | v0.7 | 4/4 | Complete    | 2026-05-31 |
| 61. Integrations surface and read-only API | v0.7 | 3/3 | Complete    | 2026-05-31 |
| 62. Key-write endpoint and verification engine | v0.7 | 3/3 | Complete    | 2026-05-31 |
| 63. Tier-1 dashboard pages, starter team, and seed content | v0.7 | 0/0 | Complete    | 2026-05-31 |
| 64. Discord control bot extension | v0.7 | 3/3 | Complete    | 2026-05-31 |
| 65. Supabase sync loop and schema migrations | v0.7 | 4/4 | Complete    | 2026-05-31 |
